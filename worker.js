 ...
 *   POST /admin     { uid, tok, act, ... }          -> founder-only. act:
 *                                                    "unlock" spends an
 *                                                    ADMIN_KEYS key once to
 *                                                    mark this uid a
 *                                                    founder; every other
 *                                                    act (stats, dump,
 *                                                    approve, block, ...)
 *                                                    just needs the normal
 *                                                    session, same as any
 *                                                    other endpoint
 *
...
// Which Google accounts (uids) run the admin panel, and the display name to
// log actions under. A single KV value: { "<uid>": "founder name", ... }.
// Populated by the "unlock" admin action, never edited by hand.
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (p === "/data" && request.method === "POST") {
      ...
      const [offers, wants, approved, blocked, doneRaw, founders] = await Promise.all([
        ...
        uid ? env.BOOKS.get(kFounders, "json") : null,
      ]);
      ...
      return json({
        ...
        me: uid ? { verified: approved.includes(uid), blocked: false, founder: (founders && founders[uid]) || null } : null,
    ...
    /* ---------------- admin ----------------
       ...
       in with — there's no separate site or separate login anymore. The
       ADMIN_KEYS secret still matters, but only once per founder: the
       "unlock" action below spends a key to permanently mark a uid as a
       founder in KV (kFounders), and every other action here just checks
       that KV record via the normal uid/tok session, same as /publish or
       /update do. Rotate a compromised key with:
         npx wrangler secret put ADMIN_KEYS
       and revoke the founder it already unlocked with "removeFounder" —
       rotating the key alone does not undo an unlock that already happened.
       ------------------------------------------------------------------ */
    if (p === "/admin" && request.method === "POST") {
      const b = await request.json().catch(() => ({}));

      /* One-time bootstrap: a Google-signed-in account spends a key from
         ADMIN_KEYS to become a founder. After this, the key is never
         needed again on this account — the uid itself is what grants
         access, exactly like every other endpoint. */
      if (b.act === "unlock") {
        if (!(await auth(env, b.uid, b.tok))) return json({ error: "auth" }, 401);
        let keyMap = {};
        ...
        for (const k of Object.keys(keyMap)) {
          if (sameHash(k, givenKey)) { grantedName = keyMap[k]; break; }
        }
        if (!grantedName) return json({ error: "key" }, 401);
        const founders = (await env.BOOKS.get(kFounders, "json")) || {};
        founders[b.uid] = grantedName;
        await env.BOOKS.put(kFounders, JSON.stringify(founders));
        await logAction(env, grantedName, "unlock", b.uid);
        return json({ ok: true, name: grantedName });
      }

      // Every other action requires an already-unlocked founder account.
      if (!(await auth(env, b.uid, b.tok))) return json({ error: "auth" }, 401);
      const founders = (await env.BOOKS.get(kFounders, "json")) || {};
      const founderName = founders[b.uid];
      if (!founderName) return json({ error: "notFounder" }, 403);

      if (b.act === "removeFounder") {
        delete founders[b.target];
        await env.BOOKS.put(kFounders, JSON.stringify(founders));
        await logAction(env, founderName, "removeFounder", b.target);
        return json({ ok: true });
      }
      ...
      if (b.act === "stats") {
        ...
        return json({
          days: dayList, totals, daily, byVillage,
      ...
      if (b.act === "approve" || b.act === "unapprove") {
        ...
        await logAction(env, founderName, b.act, b.target);
        return json({ ok: true });
      }
      ...
      if (b.act === "resetDevice") {
        ...
        await logAction(env, founderName, "resetDevice", b.target);
        return json({ ok: true });
      }
      ...
      if (b.act === "purgeUser") {
        ...
        await logAction(env, founderName, "purgeUser", b.target);
        return json({ ok: true });
      }
      ...
      if (b.act === "addNote") {
        ...
        await env.BOOKS.put(kUser(b.target), JSON.stringify(rec));
        await logAction(env, founderName, "addNote", b.target, text);
        return json({ ok: true });
      }
