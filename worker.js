...
export default {
  async fetch(request, env) {
    ...
    if (p === "/session" && request.method === "POST") {
      ...
      if (b.mode === "signup") {
        // A phone number can hold more than one account — a shared family
        // phone is normal here. What it can NOT hold is two accounts with
        // the same password: that would make the "pick your name" step at
        // sign-in cosmetic, since either password would unlock either
        // identity. Block on that, not on the number being taken.
