...
export default {
  async fetch(request, env) {
    ...
    if (p === "/session" && request.method === "POST") {
      ...
      if (b.mode === "signup") {
        // One account per phone number. A number that already has an
        // account gets a plain, unexplained failure here rather than a
        // "this number is taken" message — deliberately not confirming to
        // the caller whether a number is already registered.
        if (house.length) return json({ ok: false, needName: false, blocked: true });
      ...
      if (b.mode === "signin") {
        // Exactly one account can ever exist on a number now, so this is
        // always a direct lookup — no "which account" step needed.
        if (!house.length) return json({ ok: false, noAccount: true });
        const target = house[0];
      ...
      if (b.mode === "setpass") {
        ...
        // No other account can share this phone anymore, so there is no
        // one left to collide passwords with.
        const salt = newSalt();
