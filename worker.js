 ...
 *                                                    is either OWNER_EMAIL
 *                                                    (the one immutable
 *                                                    owner, set only via
 *                                                    secret + redeploy) or
 *                                                    on the "founders" KV
 *                                                    list, which any
 *                                                    existing founder can
 *                                                    add to from inside the
 *                                                    admin panel itself —
 *                                                    see act:"addFounder"
 *                                                    and act:"removeFounder"
 *                                                    below. Removing is
 *                                                    owner-only, and the
 *                                                    owner can never be
 *                                                    removed by anyone,
 ...
 *
 * Secrets: VERIFY_TOKEN, APP_SECRET, OWNER_EMAIL,
 ...
 * (PEPPER, ADMIN_KEYS, and ADMIN_EMAILS are no longer read anywhere —
 * password auth, the old key-based founder unlock, and the old
 * single-secret founder list are all gone, replaced by OWNER_EMAIL plus
 * the "founders" KV list above. Safe to leave any of the three secrets
 * set in Cloudflare, or remove them, either way.)
...
/**
 * Founder status now has two tiers, checked fresh on every call, nothing
 * cached and nothing to unlock:
 *   1. The owner — one email, OWNER_EMAIL, set once via
 *      `npx wrangler secret put OWNER_EMAIL` and redeploy. This is the
 *      only founder-related thing NOT editable from inside the app: no
 *      /admin action ever reads or writes it, so nobody — including the
 *      owner themself — can remove or reassign it from the admin panel.
 *      A rogue or compromised founder account can at most be removed by
 *      the owner; the owner can only be changed by whoever controls the
 *      Cloudflare account.
 *   2. Everyone else — the "founders" KV list, an array of
 *      {email, name, addedBy, addedAt, canAnalytics}, managed entirely
 *      in-app: any existing founder (owner or not) can add to it
 *      (act:"addFounder"), only the owner can remove from it
 *      (act:"removeFounder"). Same shape and spirit as the
 *      "approved"/"blocked" lists above.
 *
 * canAnalytics is a second, narrower tier on top of founder status itself
 * — every founder can moderate listings and manage users, but the
 * click/page-time insights (see queryAnalytics() and act:"stats" below)
 * are visible only to the owner (always) and to founders the owner has
 * explicitly granted it to (act:"grantAnalytics"/"revokeAnalytics",
 * owner-only either direction). New founders start without it; inviting
 * someone and trusting them with that data are two separate decisions.
 *
 * Returns a display name if `email` is a founder by either route, or ""
 * if not. The owner's display name comes from their own Google profile
 * (myRec.name, passed in by the caller) since the owner never types a
 * name into the founders list the way an invited founder does.
 */
async function founderName(env, email, myRec) {
  if (!email) return "";
  const lower = String(email).toLowerCase();
  if (env.OWNER_EMAIL && lower === String(env.OWNER_EMAIL).toLowerCase()) {
    return (myRec && myRec.name) || lower;
  }
  const founders = await readList(env.BOOKS, "founders");
  const f = founders.find((x) => String(x.email).toLowerCase() === lower);
  return f ? f.name : "";
}
function isOwnerEmail(env, email) {
  return !!env.OWNER_EMAIL && !!email && String(email).toLowerCase() === String(env.OWNER_EMAIL).toLowerCase();
}
// See canAnalytics in the doc comment above. The owner is implicitly
// always allowed — checked by the caller, not here, since that check is
// free (no KV read) and this one isn't.
async function founderCanAnalytics(env, email) {
  if (!email) return false;
  const lower = String(email).toLowerCase();
  const founders = await readList(env.BOOKS, "founders");
  const f = founders.find((x) => String(x.email).toLowerCase() === lower);
  return !!(f && f.canAnalytics);
}
...
function view(r, uid) {
  ...
  const out = {
    ...
    ver: !!r.ver, mine, held,
    // A count, never the list of who — liking is a signed-in-only action
    // (see /update above) but the count itself is public, same as it
    // would be on any shelf.
    likeCount: likes.length, liked: !!uid && likes.includes(uid),
  };
  if (uid) {
    out.name = r.name || "";
  ...
  async fetch(request, env, ctx) {
    ...
    if (p === "/data" && request.method === "POST") {
      ...
      return json({
        ...
        me: uid ? { verified: approved.includes(uid), blocked: false,
                     founder: (await founderName(env, myRec && myRec.email, myRec)) || null } : null,
    ...
    if (p === "/update" && request.method === "POST") {
      ...
      if (act === "del") {
        ...
        // Content only, and only while still open — a book already mid
        // handoff shouldn't change shape under the person holding it.
        // Contact info is never set directly here: handoff="direct" just
        // pulls whatever phone is on the owner's profile right now (see
        // /profile), same rule /publish already follows.
        if (!owner) return json({ error: "forbidden" }, 403);
        if (st !== "open") return json({ error: "unavailable" }, 409);
        if (typeof b.grade === "string") r.grade = b.grade.slice(0, 2);
        if (typeof b.subject === "string") r.subject = b.subject.slice(0, 40);
        if (typeof b.title === "string") r.title = b.title.slice(0, 90);
        if (typeof b.cond === "string") r.cond = b.cond.slice(0, 20);
        if (typeof b.desc === "string") r.desc = b.desc.slice(0, 300);
        if (typeof b.school === "string") r.school = b.school.slice(0, 60);
        if (b.handoff === "direct" || b.handoff === "point") {
          r.handoff = b.handoff;
          const rec = await env.BOOKS.get(kUser(b.uid), "json");
          r.phone = r.handoff === "direct" ? ((rec && rec.phone) || "") : "";
        }
      } else if (act === "like" || act === "unlike") {
        // No owner/holder restriction — anyone signed in can like anyone's
        // listing, including their own. One vote per uid, toggled; see
        // view()'s likeCount/liked for how this comes back out. Visible to
        // everyone (even anonymous browsers get the count), only the
        // toggle itself needs a session, same bar as reserving a book.
        const likes = Array.isArray(r.likes) ? r.likes : [];
        const has = likes.indexOf(b.uid) > -1;
        if (act === "like" && !has) likes.push(b.uid);
        if (act === "unlike" && has) likes.splice(likes.indexOf(b.uid), 1);
        r.likes = likes;
      } else {
    ...
    /* ---------------- admin ----------------
       ...
       type in. The check runs fresh on every request against the caller's
       own profile record (see /profile — email comes from Google at signup
       and never changes). See founderName()'s doc comment above for the
       full owner-vs-founders-list model; in short, everyone but the owner
       is managed from inside this route (act:"addFounder"/"removeFounder"
       below), the owner only from the Cloudflare dashboard:
    ...
    if (p === "/admin" && request.method === "POST") {
      ...
      const myName = await founderName(env, myRec && myRec.email, myRec);
      if (!myName) return json({ error: "notFounder" }, 403);
      const isOwner = isOwnerEmail(env, myRec && myRec.email);
      const myAnalytics = isOwner || (await founderCanAnalytics(env, myRec && myRec.email));
      ...
      if (b.act === "stats") {
        if (!myAnalytics) return json({ error: "noAnalyticsPerm" }, 403);
      ...
      if (b.act === "dump") {
        const [offers, wants, userKeys, log, founders] = await Promise.all([
          readList(env.BOOKS, kList(village, "offers")),
          readList(env.BOOKS, kList(village, "wants")),
          env.BOOKS.list({ prefix: "user:" }),
          readList(env.BOOKS, "log:actions"),
          readList(env.BOOKS, "founders"),
        ]);
        ...
        return json({ offers, wants, users, approved, blocked, me: myName,
                      ...
                      // Team management (see founderName()'s doc comment above).
                      // ownerEmail is shown so every founder can see who holds
                      // that permanent seat; isOwner gates the removeFounder
                      // button client-side (the /admin route enforces it too,
                      // this is just so the button doesn't render for nobody).
                      founders, isOwner, ownerEmail: env.OWNER_EMAIL || "" });
      }
      ...
      // A founder-side version of /update's "release" — that one is
      // owner/holder-only by design (see the doc there), which is exactly
      // wrong when the two people involved have gone silent on each other
      // and a book is stuck reserved for no reason. This is the escape
      // hatch: any founder can force it back open.
      if (b.act === "forceRelease") {
        const kind = b.kind === "wants" ? "wants" : "offers";
        const key = kList(village, kind);
        const list = await readList(env.BOOKS, key);
        const i = list.findIndex((x) => x.id === b.id);
        ...
        r.resAt = 0; r.confOwner = false; r.confHolder = false;
        await put(key, list);
        await logAction(env, myName, "forceRelease", b.id, r.title || r.subject || "");
        return json({ ok: true });
      }
      ...
      /* --- team management: who else can open this panel. See
             founderName()'s doc comment above for the two-tier model. --- */
      if (b.act === "addFounder") {
        const email = String(b.target || "").trim().toLowerCase();
        const name = String(b.name || "").replace(/\s+/g, " ").trim().slice(0, 80);
        if (!email || !email.includes("@") || email.length > 200) return json({ error: "badEmail" }, 400);
        if (!name) return json({ error: "badName" }, 400);
        if (isOwnerEmail(env, email)) return json({ error: "alreadyOwner" }, 400);
        const founders = await readList(env.BOOKS, "founders");
        if (founders.some((f) => String(f.email).toLowerCase() === email))
          return json({ error: "alreadyFounder" }, 400);
        founders.push({ email, name, addedBy: myName, addedAt: Date.now(), canAnalytics: false });
        await put("founders", founders);
        await logAction(env, myName, "addFounder", email, name);
        return json({ ok: true });
      }

      if (b.act === "removeFounder") {
        if (!isOwner) return json({ error: "ownerOnly" }, 403);
        const email = String(b.target || "").trim().toLowerCase();
        // The owner seat isn't in this list to begin with (see
        // founderName()), so this is a belt-and-suspenders check, not the
        // only thing standing between the owner and removal.
        if (isOwnerEmail(env, email)) return json({ error: "cantRemoveOwner" }, 400);
        const founders = await readList(env.BOOKS, "founders");
        const next = founders.filter((f) => String(f.email).toLowerCase() !== email);
        await put("founders", next);
        await logAction(env, myName, "removeFounder", email);
        return json({ ok: true });
      }

      // Second, narrower tier on top of founder status — see
      // founderCanAnalytics()'s doc comment above. Owner-only in either
      // direction: granting it is still a decision about who sees what,
      // not an invite-time nicety any founder should be able to hand out.
      if (b.act === "grantAnalytics" || b.act === "revokeAnalytics") {
        if (!isOwner) return json({ error: "ownerOnly" }, 403);
        const email = String(b.target || "").trim().toLowerCase();
        const founders = await readList(env.BOOKS, "founders");
        const idx = founders.findIndex((f) => String(f.email).toLowerCase() === email);
        if (idx < 0) return json({ error: "gone" }, 404);
        founders[idx].canAnalytics = b.act === "grantAnalytics";
        await put("founders", founders);
        await logAction(env, myName, b.act, email);
        return json({ ok: true });
      }
