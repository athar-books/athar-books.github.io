 ...
 *   POST /publish   { village, kind, uid, tok, records[], pics{} }  record.cat
 *                                                    is "book" (school,
 *                                                    default), "stat"
 *                                                    (school supplies),
 *                                                    "novel" (legacy, no
 *                                                    longer posted from the
 *                                                    UI, still read), or
 *                                                    "gen" (general library
 *                                                    — fiction/non-fiction,
 *                                                    not school-tied, uses
 *                                                    record.age instead of
 *                                                    grade; see effCat() in
 *                                                    the front end)
 ...
 *                                                    is OWNER_EMAIL or is on
 *                                                    the kFounders() KV list
 *                                                    — checked fresh on
 *                                                    every call, see
 *                                                    founderName()
 *
 ...
 *          OWNER_NAME — display name for OWNER_EMAIL; falls back to a
 ...
 * password auth and the old secret-list founder model are both gone,
 * replaced by OWNER_EMAIL plus the "founders" KV list (see founderName()).
 * Safe to leave any of the three old secrets set in Cloudflare, or remove
 * them, either way.)
...
// Founders other than the owner. One JSON array: {email, name, addedBy, addedAt}.
// Read/written the same way "approved"/"blocked" already are — see /admin.
const kFounders = () => "founders";

// Founder status has two tiers now. OWNER_EMAIL is a secret, set once
// outside the code — that account is a founder forever and the only one
// who can remove other founders (see the "removeFounder" action). Every
// other founder lives in the kFounders() KV list instead of a secret, so
// founders can add/remove teammates from inside the control panel without
// a redeploy. Async because the KV list needs a read; every call site
// awaits it.
async function founderName(env, email) {
  if (!email) return "";
  const e = String(email).toLowerCase();
  if (env.OWNER_EMAIL && e === String(env.OWNER_EMAIL).toLowerCase()) {
    return env.OWNER_NAME || "المؤسس";
  }
  const founders = await readList(env.BOOKS, kFounders());
  const hit = founders.find((f) => String(f.email || "").toLowerCase() === e);
  return hit ? hit.name : "";
}
function isOwnerEmail(env, email) {
  return !!(env.OWNER_EMAIL && email &&
    String(email).toLowerCase() === String(env.OWNER_EMAIL).toLowerCase());
}
...
function view(r, uid) {
  ...
  const out = {
    ...
    // cat was missing from this output entirely until now — every record's
    // category was being re-derived on the client purely by matching
    // `subject` against the STAT/NOVEL label strings (see effCat() in the
    // front end), which happened to still work because those two
    // categories always get a unique fixed subject string at publish time.
    // It silently breaks for any category that doesn't have one, which is
    // exactly what "gen" (general library) is, so this needed fixing for
    // that feature to work at all — not just a cosmetic gap.
    cat: r.cat || "book",
  ...
  // "gen" (general library: fiction/non-fiction, not tied to school) is the
  // only category that reads age — harmless empty string for every other
  // record. Not sensitive, so it goes out to anonymous callers too, same as
  // grade/subject.
  if (r.cat === "gen") out.age = r.age || "";
  if (uid) {
    out.name = r.name || "";
...
export default {
  async fetch(request, env, ctx) {
    ...
    // Scoped to this one request, not a shared module-level variable — a
    // Worker isolate can interleave multiple in-flight requests on the same
    // event loop, and every response in this file used to read a single
    // mutable `CORS` binding that any concurrently-running request could
    // reassign between this line and the eventual json() call. Two requests
    // racing could each end up with the other's CORS header. Closing over a
    // local const instead means each request's response headers are fixed
    // the instant this line runs, no matter what else is in flight.
    const corsHeaders = cors(request);
    const json = (d, s = 200) =>
      new Response(JSON.stringify(d), { status: s, headers: { "Content-Type": "application/json", ...corsHeaders } });
    ...
    if (p === "/data" && request.method === "POST") {
      ...
      return json({
        ...
        me: uid ? { verified: approved.includes(uid), blocked: false,
                     founder: (await founderName(env, myRec && myRec.email)) || null,
                     // Only the owner sees remove buttons in the founders
                     // list — everything else about the panel is identical
                     // for every founder. See isOwnerEmail().
                     isOwner: isOwnerEmail(env, myRec && myRec.email),
                     // Own code only, and only while it's still useful — once
                     // verified there's nothing left to text. Safe to hand
                     // back as-is: it's deterministic from the uid, and the
                     // real trust anchor is Meta confirming which WhatsApp
                     // number actually sent the message (see /webhook), not
                     // secrecy of this code.
                     code: (myRec && myRec.phone && !approved.includes(uid)) ? vCode(uid) : null } : null,
    ...
    if (p === "/publish" && request.method === "POST") {
      ...
      const now = Date.now();
      // cat was never captured here before — every published record silently
      // dropped it, so "stationery" filtering on the frontend has never
      // actually matched anything real (it reads r.cat back from /data, and
      // r.cat was always undefined). Whitelisted, not just String()'d, since
      // this is the one field the frontend uses to change how a record
      // renders — no reason to let an unrecognized value through.
      const clean = recs.map((r, i) => ({
        ...
        // "gen" = general library: fiction/non-fiction books and novels not
        // tied to school at all — no grade, its own age field instead. See
        // the front end's GEN_FICTION/GEN_NONFIC constants for what ends up
        // in `subject` for this category, same pattern "stat" and "novel"
        // already used before it.
        cat: (r.cat === "stat" || r.cat === "novel" || r.cat === "gen") ? r.cat : "book",
        ...
        // Only meaningful for cat "gen" — who the book suits. Whitelisted
        // like cat itself; anything else collapses to "" rather than being
        // let through as-is.
        age: (r.age === "kids" || r.age === "adult" || r.age === "any") ? r.age : "",
        name: rec.name,
    ...
    if (p === "/webhook" && request.method === "POST") {
      ...
      for (const m of messages) {
        ...
        if (!approved.includes(uid)) {
          ...
          // Self-verified over WhatsApp, not by a founder — still worth
          // recording on the account so the panel doesn't show a blank
          // "verified by" next to someone who never had one, same field
          // /admin's "approve" action fills in for founder-driven approvals.
          const rec = await env.BOOKS.get(kUser(uid), "json");
          if (rec) {
            rec.verifiedBy = "تحقق ذاتيًا عبر واتساب";
    ...
    /* ---------------- admin ----------------
       ...
       and never changes): OWNER_EMAIL is a founder forever, and everyone
       else is whoever's currently in the kFounders() KV list — see
       founderName() and the addFounder/removeFounder actions below for how
       that list is managed from inside the panel itself now.
       ------------------------------------------------------------------ */
    if (p === "/admin" && request.method === "POST") {
      const b = await request.json().catch(() => ({}));
      if (!(await auth(env, b.uid, b.tok))) return json({ error: "auth" }, 401);
      const myRec = await env.BOOKS.get(kUser(b.uid), "json");
      const myEmail = myRec && myRec.email;
      const myName = await founderName(env, myEmail);
      ...
      if (b.act === "dump") {
        ...
        const users = await Promise.all(userKeys.keys.map(async (k) => {
          ...
          return { uid, name: u.name || "", phone: u.phone || "", at: u.at || 0,
                   ...
                   verifiedBy: u.verifiedBy || "", verifiedAt: u.verifiedAt || 0,
                   notes: Array.isArray(u.notes) ? u.notes
                        : (u.note ? [{ by: "", text: u.note, at: u.at || 0 }] : []) };
        }));
        // Most recent first everywhere in the panel — newest signup, newest
        // founder added, on top; oldest at the bottom. offers/wants are
        // already newest-first as stored (see /publish, which prepends);
        // users and founders aren't naturally ordered that way, so sort
        // both explicitly here rather than leaving it to the client.
        users.sort((a, b) => (b.at || 0) - (a.at || 0));
        founders.sort((a, b) => (b.addedAt || 0) - (a.addedAt || 0));
        return json({ offers, wants, users, approved, blocked, me: myName,
                      log: log.slice(-100).reverse(),
                      founders, isOwner: isOwnerEmail(env, myEmail),
                      ownerName: env.OWNER_NAME || "المؤسس" });
      }
      ...
      if (b.act === "approve" || b.act === "unapprove") {
        ...
        // Recorded on the user's own record, not just the shared action
        // log, so the panel can show "verified by X" on their row directly
        // — see adminUserRow() in index.html — without scanning history
        // that log:actions eventually rolls off (see logAction()'s 300 cap).
        const rec = await env.BOOKS.get(kUser(b.target), "json");
        if (rec) {
          rec.verifiedBy = b.act === "approve" ? myName : "";
          rec.verifiedAt = b.act === "approve" ? Date.now() : 0;
      ...
      /* --- founder management: OWNER_EMAIL is fixed outside the code;
             everyone else lives in kFounders() and can be managed from
             here. Any founder can add a teammate; only the owner can
             remove one — that split is enforced here, not just hidden in
             the UI, since the frontend's isOwner check is a display
             convenience and nothing more. --- */
      if (b.act === "addFounder") {
        const email = String(b.email || "").trim().toLowerCase();
        const name = String(b.name || "").trim().slice(0, 60);
        if (!email || !email.includes("@") || !name) return json({ error: "invalid" }, 400);
        if (isOwnerEmail(env, email)) return json({ error: "isOwner" }, 400);
        const founders = await readList(env.BOOKS, kFounders());
        if (founders.some((f) => String(f.email || "").toLowerCase() === email)) {
          return json({ error: "exists" }, 400);
        }
        founders.push({ email, name, addedBy: myName, addedAt: Date.now() });
        await put(kFounders(), founders);
        await logAction(env, myName, "addFounder", email, name);
        return json({ ok: true });
      }

      if (b.act === "removeFounder") {
        if (!isOwnerEmail(env, myEmail)) return json({ error: "ownerOnly" }, 403);
        const email = String(b.target || "").trim().toLowerCase();
        if (isOwnerEmail(env, email)) return json({ error: "isOwner" }, 400);
        const founders = await readList(env.BOOKS, kFounders());
        await put(kFounders(), founders.filter((f) => String(f.email || "").toLowerCase() !== email));
